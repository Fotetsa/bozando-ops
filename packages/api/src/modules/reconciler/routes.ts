...
export async function registerReconcilerRoutes(app: FastifyInstance) {
  ...
  app.post("/api/projects/:id/deploy", operator, async (req, reply) => {
    const { id } = req.params as { id: string }
    ...
    try {
      ...
      await eventBus.emit("deploy.finished", { projectId: id, userId, ok: true, log })
      return { ok: true, log }
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      await projectsService.updateProject(id, { status: "error" })
      await eventBus.emit("deploy.finished", { projectId: id, userId, ok: false, error: message })
  ...
  app.post("/api/projects/:id/destroy", operator, async (req, reply) => {
    const { id } = req.params as { id: string }
    const graph = await projectsService.getProjectGraph(id)
    if (!graph) return reply.code(404).send({ error: "project not found" })
    ...
    await eventBus.emit("destroy.finished", { projectId: id, userId: currentUser(req)?.sub, log })
    return { ok: true, log }
  })

  // Reconstruire le désiré depuis Docker (résilience Postgres perdu).
  app.post("/api/rebuild-from-docker", operator, async () => {
  ...
  app.get("/api/prune", operator, async () => {
    return pruneOrphans(false)
  })
  app.post("/api/prune", owner, async () => {
