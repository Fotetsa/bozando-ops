...
export async function registerSecretsRoutes(app: FastifyInstance) {
  ...
  app.get("/api/secrets", operator, async () => {
  ...
  app.post("/api/secrets", operator, async (req, reply) => {
    ...
    await eventBus.emit("secret.set", { userId: currentUser(req)?.sub, name })
    return { ok: true, name }
  })

  app.delete("/api/secrets/:name", operator, async (req, reply) => {
    ...
    await eventBus.emit("secret.removed", { userId: currentUser(req)?.sub, name })
    return { ok: true }
  })
}
